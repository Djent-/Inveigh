...
{
    "[-] $($invalid_parameter) is not a valid parameter" | Write-Host
    throw
}
...
{ 
    ...
    {
        "[-] Error finding local IP, specify manually with -IP" | Write-Host
        throw
    }
...
if($HTTPDefaultFile -or $HTTPDefaultEXE)
{
    ...
    if(!$HTTPDirectory)
    {
        "[-] You must specify an -HTTPDir when using either -HTTPDefaultFile or -HTTPDefaultEXE" | Write-Host
        throw
    }
...
{
    "[-] You must specify a -KerberosCredential or -KerberosHash when enabling Kerberos capture" | Write-Host
    throw
}
...
if($WPADIP -or $WPADPort)
{
    ...
    if(!$WPADIP)
    {
        "[-] You must specify a -WPADPort to go with -WPADIP" | Write-Host
        throw
    }

    if(!$WPADPort)
    {
        "[-] You must specify a -WPADIP to go with -WPADPort" | Write-Host
        throw
    }
...
{
    "[-] You must specify a -NBNSBruteForceTarget if enabling -NBNSBruteForce" | Write-Host
    throw
}
...
if($inveigh.running)
{
    "[-] Inveigh is already running" | Write-Host
    throw
}
...
if(!$elevated_privilege)
{
    ...
    if($HTTPS -eq 'Y')
    {
        "[-] HTTPS requires elevated privileges" | Write-Host
        throw
    }

    if($SpooferLearning -eq 'Y')
    {
        "[-] SpooferLearning requires elevated privileges" | Write-Host
        throw
    }

    if($Pcap -eq 'File')
    {
        "[-] Pcap file output requires elevated privileges" | Write-Host
        throw
    }
...
while($inveigh.output_queue.Count -gt 0)
{
    ...
    switch -Wildcard ($inveigh.output_queue[0])
    {
        ...
        {$_ -like "?`[`!`]*" -or $_ -like "?`[-`]*"}
        {
            ...
            {
                ($inveigh.output_queue[0] + $inveigh.newline) | Write-Host
            }
        ...
        default
        {
            ...
            elseif($inveigh.status_output)
            {
                ($inveigh.output_queue[0]) | Write-Host
            }
    ...
    if($ConsoleOutput -ne 'N')
    {
        ...
        :console_loop while(($inveigh.running -and $inveigh.console_output) -or ($inveigh.console_queue.Count -gt 0 -and $inveigh.console_output))
        {
            ...
            while($inveigh.console_queue.Count -gt 0)
            {
                ...
                switch -wildcard ($inveigh.console_queue[0])
                {
                    ...
                    {$_ -like "?`[`!`]*" -or $_ -like "?`[-`]*"}
                    {
                        ...
                        {
                            ($inveigh.console_queue[0] + $inveigh.newline) | Write-Host
                        }
                        else
                        {
                    ...
                    {$_ -like "* spoofer disabled" -or $_ -like "* local request" -or $_ -like "* host header *" -or $_ -like "* user agent received *"}
                    {
                        ...
                        if($ConsoleOutput -eq 'Y')
                        {
                            ...
                            {
                                ($inveigh.console_queue[0]) | Write-Host
                            }
            ...
            if($ConsoleStatus -and $console_status_stopwatch.Elapsed -ge $console_status_timeout)
            {
                ...
                {
                    ("[*] [$(Get-Date -format s)] Current unique cleartext captures:" + $inveigh.newline) | Write-Host
                    ...
                    foreach($unique_cleartext in $cleartext_list_temp)
                    {
                        ...
                        {
                            ($unique_cleartext + $inveigh.newline) | Write-Host
                        }
                ...
                {
                    ("[+] [$(Get-Date -format s)] No cleartext credentials have been captured" + $inveigh.newline) | Write-Host
                }

                if($inveigh.POST_request_list.Count -gt 0)
                {
                    ("[*] [$(Get-Date -format s)] Current unique POST request captures:" + $inveigh.newline) | Write-Host
                    ...
                    foreach($unique_POST_request in $POST_request_list_temp)
                    {
                        ...
                        {
                            ($unique_POST_request + $inveigh.newline) | Write-Host
                        }
                ...
                {
                    ("[*] [$(Get-Date -format s)] Current unique NTLMv1 challenge/response captures:" + $inveigh.newline) | Write-Host
                    ...
                    foreach($unique_NTLMv1 in $NTLMv1_list_temp)
                    {
                        ...
                        {
                            ($unique_NTLMv1 + $inveigh.newline) | Write-Host
                        }
                    ...
                    Start-Sleep -m 5
                    ("[*] [$(Get-Date -format s)] Current NTLMv1 IP addresses and usernames:" + $inveigh.newline) | Write-Host
                    $NTLMv1_username_list_temp = $inveigh.NTLMv1_username_list

                    foreach($NTLMv1_username in $NTLMv1_username_list_temp)
                    {
                        ($NTLMv1_username + $inveigh.newline) | Write-Host
                    }
                ...
                {
                    ("[+] [$(Get-Date -format s)] No NTLMv1 challenge/response hashes have been captured" + $inveigh.newline) | Write-Host
                }

                if($inveigh.NTLMv2_list.Count -gt 0)
                {
                    ("[*] [$(Get-Date -format s)] Current unique NTLMv2 challenge/response captures:" + $inveigh.newline) | Write-Host
                    ...
                    foreach($unique_NTLMv2 in $NTLMv2_list_temp)
                    {
                        ...
                        {
                            ($unique_NTLMv2 + $inveigh.newline) | Write-Host
                        }
                    ...
                    Start-Sleep -m 5
                    ("[*] [$(Get-Date -format s)] Current NTLMv2 IP addresses and usernames:" + $inveigh.newline) | Write-Host
                    $NTLMv2_username_list_temp = $inveigh.NTLMv2_username_list

                    foreach($NTLMv2_username in $NTLMv2_username_list_temp)
                    {
                        ($NTLMv2_username + $inveigh.newline) | Write-Host
                    }
                
                }
                else
                {
                    ("[+] [$(Get-Date -format s)] No NTLMv2 challenge/response hashes have been captured" + $inveigh.newline) | Write-Host
                }
...
#>
    ...
    if($inveigh)
    {
        ...
        {
            "[-] There are no running Inveigh functions" | Write-Host
        }
...
#>
    ...
    if($KerberosTGT)
    {
        $inveigh.kerberos_TGT_list[$KerberosTGT] | Write-Host
    }

    if($KerberosUsername)
    {
        $inveigh.kerberos_TGT_username_list | Write-Host
    }

    if($Log)
    {
        $inveigh.log | Write-Host
    }

    if($NTLMv1)
    {
        $inveigh.NTLMv1_list | Write-Host
    }
    ...
    if($NTLMv1Usernames)
    {
        $inveigh.NTLMv2_username_list | Write-Host
    }

    if($NTLMv2)
    {
        $inveigh.NTLMv2_list | Write-Host
    }
    ...
    if($Cleartext)
    {
        $inveigh.cleartext_list | Write-Host
    }

    if($CleartextUnique)
    {
        $inveigh.cleartext_list | Get-Unique | Write-Host
    }

    if($POSTRequest)
    {
        $inveigh.POST_request_list | Write-Host
    }

    if($POSTRequestUnique)
    {
        $inveigh.POST_request_list | Get-Unique | Write-Host
    }

    if($Learning)
    {
        $inveigh.valid_host_list | Write-Host
    }
    ...
    if($Session)
    {
        ...
        $inveigh.session | Format-Table -AutoSize | Write-Host
    }

    if($Enumerate)
    {
        $inveigh.enumerate | Write-Host
    }
...
if($inveigh.tool -ne 1)
{
    ...
    if($inveigh.running -or $inveigh.relay_running)
    {
        ...
        if(!$NoConsoleMessage)
        {
            "[*] Press any key to stop console output" | Write-Host
        }
    ...
    {
        "[-] Inveigh isn't running" | Write-Host
    }

}
else
{
    "[-] Watch-Inveigh cannot be used with current external tool selection" | Write-Host
}
...
if($inveigh)
{
    ...
    if(!$inveigh.running -and !$inveigh.relay_running)
    {
        ...
        "[+] Inveigh data has been cleared from memory" | Write-Host
    }
    else
    {
        "[-] Run Stop-Inveigh before running Clear-Inveigh" | Write-Host
    }
...
function ConvertTo-Inveigh
{
    ...
    {
        "Specifiy a BloodHound computers, groups, or sessions JSON file" | Write-Host
        throw
    }

    if($inveigh.running -or $inveigh.relay_running)
    {
        "Run Stop-Inveigh before importing data with ConvertTo-Inveigh" | Write-Host
        throw
    }
    ...
    {       
        ...
        "[*] Parsing BloodHound Computers JSON" | Write-Host
        $stopwatch_parse = [System.Diagnostics.Stopwatch]::StartNew()
        $bloodhound_computers = Invoke-ParseItem $bloodhound_computers
        "[+] Parsing completed in $([Math]::Truncate($stopwatch_parse.Elapsed.TotalSeconds)) seconds" | Write-Host
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        "[*] Importing computers to Inveigh" | Write-Host
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0

        if(!$bloodhound_computers.Computers)
        {
            "[!] JSON computers parse failed" | Write-Host
            throw
        }
        ...
        $bloodhound_computers.Computers | ForEach-Object {
            ...
            if($DNS)
            {
                ...
                if(!$IP)
                {
                    "[-] DNS lookup for $Hostname failed" | Write-Host
                }
        ...
        "[+] Import completed in $([Math]::Truncate($stopwatch_parse.Elapsed.TotalSeconds)) seconds" | Write-Host
        $stopwatch_parse.Reset()
    ...
    if($Sessions)
    {
        ...
        $stopwatch_parse = [System.Diagnostics.Stopwatch]::StartNew()
        "[*] Parsing BloodHound Sessions JSON" | Write-Host
        $bloodhound_sessions = Invoke-ParseItem $bloodhound_sessions
        "[+] Parsing completed in $([Math]::Truncate($stopwatch_parse.Elapsed.TotalSeconds)) seconds" | Write-Host
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        "[*] Importing sessions to Inveigh" | Write-Host
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0

        if(!$bloodhound_sessions.Sessions)
        {
            "[!] JSON sessions parse failed" | Write-Host
            throw
        }
        ...
        $bloodhound_sessions.Sessions | ForEach-Object {
            ...
            else
            {
                ...
                if($DNS)
                {
                    ...
                    if(!$IP)
                    {
                        "[-] DNS lookup for $Hostname failed or IPv6 address" | Write-Host
                    }
    ...
    if($Groups)
    {
        ...
        $stopwatch_parse = [System.Diagnostics.Stopwatch]::StartNew()
        "[*] Parsing BloodHound Groups JSON" | Write-Host
        $bloodhound_groups = Invoke-ParseItem $bloodhound_groups
        "[+] Parsing completed in $([Math]::Truncate($stopwatch_parse.Elapsed.TotalSeconds)) seconds" | Write-Host
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        "[*] Importing groups to Inveigh" | Write-Host
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0

        if(!$bloodhound_groups.Groups)
        {
            "[!] JSON groups parse failed" | Write-Host
            throw
        }
        ...
        "[+] Import completed in $([Math]::Truncate($stopwatch.Elapsed.TotalSeconds)) seconds" | Write-Host
    }
